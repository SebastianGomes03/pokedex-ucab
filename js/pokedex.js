...
function fetchPokemonDetails(pokemonId) {
    ...
    request.onsuccess = function(event) {
        if (request.result) {
            ...
            fetch(`${url}${pokemonId}`).then(response => response.json()).then(data => {
                ...
                fetch(speciesUrl) .then(response => response.json()).then(speciesData => {
                	const evolutionChainUrl = speciesData.evolution_chain.url; // Obtiene la URL de la cadena de evolución.
                    fetch(evolutionChainUrl).then(response => response.json()).then(evolutionData => {
...
function updateSpriteGrandElement(pokemonData) {
    ...
    if (!pokemonData || !pokemonData.sprites || !pokemonData.sprites.other["official-artwork"]) {
...
function updateInfoBox(pokemonData, evolutionData) {
    ...
    infoBox.innerHTML = `
        ...
        <div class="full_width">
            <p>Cadena de Evolución: ${evolutionChain.join(' -> ')}</p>
        </div>
    `;
}
